...
class AddCurrencyToFavoritesUseCaseTest {
    ...
    fun useCase_localThrowException_returnErrorDataState() {
        ...
        runBlocking {
            val dataState = addCurrencyToFavoritesUseCase.invoke(currency)
            MatcherAssert.assertThat(dataState.status, IsEqual(expectedResult.status))
            MatcherAssert.assertThat(dataState.data, IsEqual(expectedResult.data))
            MatcherAssert.assertThat(dataState.errorType, IsEqual(expectedResult.errorType))
        }
    }
    ...
    fun useCase_localSuccess_returnSuccessDataState() {
        fakeTestCurrencyRepository.data =
            mutableListOf(Currency("USD"), Currency("RUB"))
        val expectedDataInLocal = mutableListOf(Currency("USD"), Currency("RUB"), Currency("EUR"))
        val addedCurrency = Currency("EUR")
        val expectedResult = DataState<Any?>(
        ...
        runBlocking {
            val dataState = addCurrencyToFavoritesUseCase.invoke(addedCurrency)
            MatcherAssert.assertThat(dataState.status, IsEqual(expectedResult.status))
            MatcherAssert.assertThat(dataState.data, IsEqual(expectedResult.data))
            MatcherAssert.assertThat(dataState.errorType, IsEqual(expectedResult.errorType))
            MatcherAssert.assertThat(fakeTestCurrencyRepository.data.toList(), IsEqual(expectedDataInLocal))
        }
    }
